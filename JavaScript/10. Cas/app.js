...
console.log("Recenica ispisana malim slovima ce biti: \n" + recenica01);
console.log("Dok ce recenica ispisana velikim slovima biti: \n" + recenica02);
...
for(let i = 0; i < recenica.length; i++) {
    if(i === recenica.length - 1 || recenica[i+1] === "." || recenica[i+1] === ",") {
    novaRecenica2 += recenica[i].toUpperCase();        
    } else if (recenica[i+1] === " ") {
    novaRecenica2 += recenica[i].toUpperCase();
    } else {
...
// 1. Prebrojati koliko se puta u unetom strinu pojavljuje slovo "M" (i veliko i malo). Npr. za string 'Mama ima momu, dobija se rezultat 5.
